...
class authService {
    ...
    googleLoginService = async (body) => {
        ...
        try {
            ...
            const payload = { mail_id: body.mail };
            const accesstoken = jwt.sign(payload, config.jwtSecret, {
                expiresIn: "720h",
            });
            console.log(checkmail?.rows[0]?.mail_id, "----------------------__>Mail")
            if (checkmail?.rows[0]?.mail_id != null || checkmail?.rows[0]?.mail_id != undefined) {
                const res = await pool.query(registration.checkreg, [body.mail])
                if (res?.rows[0]?.registration_done) {
                    return { message: "reg_done", authorization: accesstoken, name: checkmail?.rows[0]?.name }
                }
                else {
                    return { message: "reg_not_done", authorization: accesstoken, name: checkmail?.rows[0]?.name }
                }
            }
            else {
                console.log("inside else")
                const resp = await pool.query(registration.insertmail, [body.mail, body.name])
                return { message: "reg_not_done", authorization: accesstoken, name: body.name }
            }

        } catch (error) {
            console.error('error @ GoogleLogin Service', error)
            throw new ApiError(error.statusCode || 500, error)
        }
    }
    ...
    microsoftLoginService = async (body) => {
        console.log(body, "-------------------------_> body")
        try {
            const checkmail = await pool.query(registration.signupcheck, [body.mail])
            const payload = { mail_id: body.mail };
            ...
            console.error('error @ MicrosoftLogin Service', error)
            throw new ApiError(error.statusCode || 500, error)
        }
    }
